...
async function processAction(msg, cfg, snapshot = {}) {
  try {
    ...
    let contacts = await getContacts(cfg, snapshot);

    if (cfg.devMode) console.log('contacts', JSON.stringify(contacts));

    contacts = contacts.contacts; // eslint-disable-line prefer-destructuring
    ...
    for (let i = 0; i < length; i += 1) {
      ...
      if (transformedMessage.person) {
        const personMessage = {
          data: transformedMessage.person,
          metadata: transformedMessage.metadata,
        };
        this.emit('data', personMessage);
      }

      if (transformedMessage.organization) {
        const organizationMessage = {
          data: transformedMessage.organization,
          metadata: transformedMessage.metadata,
        };
