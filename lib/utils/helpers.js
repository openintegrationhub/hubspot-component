...
async function getContacts(cfg, snapshot) { // , snapshot
  try {
    ...
    const lastUpdated = parseInt(snapshot.lastUpdated, 10);
    ...
    const filter = { propertyName: 'lastmodifieddate', operator: 'GT', value: lastUpdated };
    ...
    let companyResult = await hubspotClient.crm.contacts.searchApi.doSearch(searchRequest);
    ...
    if (Array.isArray(companyResult) && companyResult.length > 0) {
      const companiesDate = companyResult[companyResult.length - 1].properties.hs_lastmodifieddate;

      const date1 = new Date(hubspotDate).getTime();
      const date2 = new Date(companiesDate).getTime();
      if (date2 > date1) hubspotDate = companiesDate;
